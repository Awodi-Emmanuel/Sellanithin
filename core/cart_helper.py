...
class CartHelper:
    ...
    def calculate_cart_base_amount(self):
        for cart_item in self.cart_items:
            self.cart_base_total_amount += cart_item.price * cart_item.quantity
        ...
        try:
            for discount in self.discounts.get('campaigns', []):
                ...
                if discount.discount_type == 'Rate':
                    self.campaign_discount_amounts.append((self.cart_base_total_amount * discount.amount.get('rate')) / 100)
                    
            for discount in self.discounts.get('coupon', []):
